...
//  Budgie
//
//  Cookie-authed REST endpoints (base /api). Thin, typed facade over APIClient.
//
...
enum RESTAPI {
    ...
    static func checkoutPlus() async throws -> CheckoutResult {
        try await APIClient.send(
            APIRequest(method: "POST", path: "/api/plus/checkout"),
            as: CheckoutResult.self
        )
    }

    static func plusStatus(orderId: String) async throws -> PlusStatus {
        try await APIClient.send(
            APIRequest(path: "/api/plus/status/\(orderId)"),
            as: PlusStatus.self
        )
    }
    ...
    static func balanceAccounts() async throws -> [BalanceAccount] {
        try await APIClient.send(
            APIRequest(path: "/api/balance-accounts"),
    ...
    static func deleteAccount(id: String) async throws {
        try await APIClient.sendNoContent(
            APIRequest(method: "DELETE", path: "/api/balance-accounts/\(id)")
        )
    }
    ...
    static func transactions() async throws -> [Transaction] {
        try await APIClient.send(
            APIRequest(path: "/api/transactions"),
            as: [Transaction].self
        )
    }
    ...
    static func deleteTransaction(id: String) async throws {
        try await APIClient.sendNoContent(
            APIRequest(method: "DELETE", path: "/api/transactions/\(id)")
        )
    }
    ...
    static func createBudget(category: String, amount: Double, periodDays: Int) async throws -> Budget {
        ...
        return try await APIClient.send(
            APIRequest(method: "POST", path: "/api/budgets", body: body),
            as: Budget.self
        )
    }
    ...
    static func updateBudget(id: String, category: String, amount: Double, periodDays: Int) async throws -> Budget {
        ...
        return try await APIClient.send(
            APIRequest(method: "PATCH", path: "/api/budgets/\(id)", body: body),
            as: Budget.self
        )
    }
    ...
    static func deleteBudget(id: String) async throws {
        try await APIClient.sendNoContent(
            APIRequest(method: "DELETE", path: "/api/budgets/\(id)")
        )
    }
    ...
    static func deleteSubscription(id: String) async throws {
        try await APIClient.sendNoContent(
            APIRequest(method: "DELETE", path: "/api/subscriptions/\(id)")
        )
    }
}
