...
import { isRateLimitError, MODEL_CHAIN } from "@/lib/chat-models";
import {
  clearChat,
  loadChatDraft,
  loadChatMessages,
...
export function ChatView({
  ...
  useEffect(() => {
    Promise.resolve().then(() => {
      ...
      setModelIndex(Math.max(0, MODEL_CHAIN.indexOf(savedModel)));
      setHydrated(true);
    });
  }, [userId, setMessages]);
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-20rem)] md:h-[calc(100vh-19rem)] gap-3">
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center justify-between gap-2">
          <p className="text-sm text-black/40">
            Ask Budgie anything about your money, or tell it to record a
            transaction.
          </p>
          {messages.length > 0 && (
            <button
              ...
              title="Clear chat"
              className="w-9 h-9 shrink-0 rounded-full flex items-center justify-center text-black/40 hover:text-black hover:bg-[#F2F2F2] transition active:scale-95"
            >
              <Trash2 className="w-4 h-4" />
            </button>
          )}
        </div>
        {hydrated && modelIndex > 0 && (
